...
#[sp_name]_vpd_controlled_25c_filtered <- 
    ...
    # Remove Zeros
    filter(if_any(strain_avg_1_microstrain_avg:strain_avg_8_microstrain_avg, ~ .x > 5)) %>% 
...
```{r, eval=FALSE}
[sp_name]_vpd_controlled_droughtbox_data_cleaned <-
...
ggplot(data = [sp_name]_vpd_controlled_droughtbox_data_cleaned, aes(x = time_seconds,
                                                                    y = string_weight_grams,
                                                                    colour = factor(string_number))) +
