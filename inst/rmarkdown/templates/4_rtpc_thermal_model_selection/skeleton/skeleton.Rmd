...
#[spcode]_model_stack_string_[NUMBER] <- 
    ...
    select(models_fits_string_[NUMBER], -data) %>%
...
#d_labs <- 
    filter(string_[NUMBER]_preds, temp <= 40) %>%
        group_by(., model_name) %>%
        sample_n(., 1) %>%
        ungroup()
```
...
# ggplot([spcode]_string_[NUMBER]_preds, aes(temp, .fitted)) +
#   
...
```{r, eval=FALSE}
#[spcode]_rmax_no_vpd_control_model <-
