...
```{r, export_data_rwc_filtered_data}
save([sp_name]_data_for_water_loss, 
     file = "./[sp_name]_processed_data/[sp_name]_rwc_filtered_data.RData")
```
...
ggplot(data = [sp_name]_transpiration_rates, aes(x = set_temperature,
        ...
        # Set colors
        #scale_color_manual(values = c("#9e1523","#293352")) +
        ylab('Transpiration (grams*m-2*s-1)')+
        xlab("Temperature") +
...
#[sp_name]_water_loss_data <- 
  ...
  select(-c(median_vpd, median_vpd, median_rh, median_temp)) %>% 
