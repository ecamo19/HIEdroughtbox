...
### Plot VPD median values

```{r}
ggplot(data = [sp_name]_residual_conductance_data, aes(x = factor(set_temperature),
                                                       y = median_vpd)) +
    geom_boxplot() +
    theme_bw(base_size = 14) +
    facet_wrap(~vpd_control) +
...
```{r, eval = FALSE}
glimpse([sp_name]_residual_conductance_data)
```
...
[sp_name]_residual_conductance_data %>%
    ...
    # Summarize data
    summarise(
        median_residual_conductance = median(double_sided_residual_conductance_mol_s_m2),
        sd_residual_conductance = sd(double_sided_residual_conductance_mol_s_m2),
