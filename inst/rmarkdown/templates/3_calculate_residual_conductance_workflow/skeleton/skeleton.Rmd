...
```{r, eval = FALSE}
# Make sure the maximum row length of the data frame slopes is 8 (individuals)  * 6 (temperature treatments) = 48. Can be less but not more!!
nrow(slopes_no_vpd_control_data)
```
...
nrow(slopes_vpd_controlled_data)
```

```{r, plot_slopes_no_vpd_control, eval = FALSE, echo = FALSE}
ggplot(data = slopes_no_vpd_control_data, aes(x = factor(set_temperature),
...
```{r, plot_slopes_vpd_controlled, eval = FALSE, echo = FALSE}
ggplot(data = slopes_no_vpd_control_data, aes(x = factor(set_temperature),
                                              y = slope_grams_per_second)) +
    geom_boxplot() +
    theme_bw() +
